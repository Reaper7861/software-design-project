...
const EventManagement = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (localDataRaw) {
                ...
                setEvents(demoEvents);
                const initialData = { events: demoEvents, users: [], matches: [] };
    ...
    return (
        <Box sx={{ maxWidth: 600, mx: 'auto', mt: 4, p: 3, border: '1px solid #ccc', borderRadius: 2, boxShadow: 3, backgroundColor: '#fafafa' }}>
            <Typography variant="h5" gutterBottom>Event Management</Typography>
            {error && <Alert severity="error" sx={{ mb: 2 }}>{error}</Alert>}
            <Box component="form" onSubmit={handleSubmit}>
                <TextField label="Event Name (100 characters)" fullWidth name="name" value={formData.name} onChange={handleChange} sx={{ mb: 2 }} slotProps={{ htmlInput: { maxLength: 100 } }} required />
                <TextField label="Event Description" fullWidth multiline rows={4} name="description" value={formData.description} onChange={handleChange} sx={{ mb: 2 }} required />
                <TextField label="Location" fullWidth multiline rows={2} name="location" value={formData.location} onChange={handleChange} sx={{ mb: 2 }} required />
                <FormControl fullWidth sx={{ mb: 2 }} required>
                    <InputLabel>Required Skills</InputLabel>
                    <Select multiple name="skills" value={formData.skills} onChange={handleSkillsChange} label="Required Skills" renderValue={(selected) => selected.join(', ')}>
                ...
                <FormControl fullWidth sx={{ mb: 2 }} required>
                    ...
                    <Select name="urgency" value={formData.urgency} onChange={handleChange} label="Urgency">
                        ...
                        {urgencyOptions.map(option => <MenuItem key={option} value={option}>{option}</MenuItem>)}
                    </Select>
                </FormControl>
                <TextField label="Event Date" type="date" fullWidth name="date" value={formData.date} onChange={handleChange} sx={{ mb: 2 }} InputLabelProps={{ shrink: true }} required />
                <Button variant="contained" color="secondary" fullWidth type="submit" disabled={loading} sx={{ mb: 2 }}>
                    {loading ? 'Saving...' : formData.id ? 'Update Event' : 'Create Event'}
                </Button>
            </Box>
            <Typography variant="h6" gutterBottom sx={{ mt: 4 }}>Existing Events</Typography>
            {events.length === 0 ? (
                        ...
                        {events.map((event, index) => (
                            <React.Fragment key={event.id}>
                                <ListItem
                                    secondaryAction={
                                        <Box>
                                            <Button variant="outlined" color="secondary" size="small" onClick={() => handleEdit(event)} sx={{ mr: 1 }}>Edit</Button>
                                            <Button variant="outlined" color="error" size="small" onClick={() => handleDelete(event.id)}>Delete</Button>
                                        </Box>
                                    }
                                >
                                    <ListItemText primary={event.name} secondary={`Date: ${event.date} | Skills: ${event.skills.join(', ')}`} />
                                </ListItem>
